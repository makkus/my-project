...
black: ## run black
	black --config pyproject.toml setup.py src/my_project tests

flake: ## check style with flake8
	flake8 src/my_project tests

mypy: ## run mypy
	mypy --namespace-packages --explicit-package-bases src/my_project
...
attribution: ## create a json file with attribution information
	scripts/attribution/create_attribution_data.sh . ./src/my_project/resources/attribution.json
